		...
		FROM
			cte AS g
				...
				ON g.car_group_uuid = c.parent_uuid
					{$manufacturer	!== null ? 'AND c.manufacturer_uuid = :manufacturer_uuid'	: ''}
					{$model			!== null ? 'AND c.model_uuid = :model_uuid'					: ''}
					{$modification	!== null ? 'AND c.modification_uuid = :modification_uuid'	: ''}
				INNER JOIN properties_values AS v
				ON c.{$manufacturer === null ? 'manufacturer' :
						($model === null ? 'model' :
							($modification === null ? 'modification' : 'year')) }_uuid = v.uuid
EOT
		;
