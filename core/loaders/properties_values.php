...
////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------
class properties_values_loader extends objects_loader {
	...
	public function load_objects() {
		...
		$all_fields = [ 'uuid', 'marked', 'vr', 'code', 'property_uuid', 'value_type', 'value_b', 'value_n', 'value_s' ];
		$fields = [];
		$fields_uuid = [];
		...
		foreach( $this->objects_ as $object ) {
			...
			if( @$erase ) {
				...
				if( $st_erase === null ) {
					...
					$st_erase = $this->infobase_->prepare('DELETE FROM properties_values WHERE uuid = :uuid AND vr IS NULL');
				...
				if( $st === null ) {
					...
					$st = $this->infobase_->prepare("REPLACE INTO properties_values (${gf}) VALUES (:${gv})");
		...
		if( config::$analyze_sqlite_tables )
			$this->infobase_->exec('ANALYZE properties_values');
		...
		if( config::$log_timing ) {
		    ...
		    error_log(sprintf('%u', $cnt) . ' properties values updated, ' . $rps . ' rps, ellapsed: ' . $timer->ellapsed_string($ellapsed));
