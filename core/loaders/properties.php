...
////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------
class properties_loader extends objects_loader {
	...
	public function load_objects() {
		...
		$all_fields = [ 'uuid', 'marked', 'code', 'name', 'assignment_uuid' ];
		$fields = [];
		$fields_uuid = [];
		...
		foreach( $this->objects_ as $object ) {
			...
			if( @$erase ) {
				...
				if( $st_erase === null ) {
					...
					$st_erase = $this->infobase_->prepare('DELETE FROM properties WHERE uuid = :uuid');
				...
				if( $st === null ) {
					...
					$st = $this->infobase_->prepare("REPLACE INTO properties (${gf}) VALUES (:${gv})");
		...
		if( config::$analyze_sqlite_tables )
			$this->infobase_->exec('ANALYZE properties');
		...
		if( config::$log_timing ) {
	    	...
	    	error_log(sprintf('%u', $cnt) . ' properties updated, ' . $rps . ' rps, ellapsed: ' . $timer->ellapsed_string($ellapsed));
