...
class properties_assignments_loader extends objects_loader {
	...
	public function load_objects() {
		...
		if( config::$analyze_sqlite_tables )
			$this->infobase_->exec('ANALYZE properties_assignments');
		...
		if( config::$log_timing ) {
		    ...
		    error_log(sprintf('%u', $cnt) . ' properties assignments updated, ' . $rps . ' rps, ellapsed: ' . $timer->ellapsed_string($ellapsed));
