...
////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------
class products_selection_by_properties_setup_registry_loader extends objects_loader {
...
EOT
			...
			foreach( $recordset as $record ) {
				...
				if( $st === null ) {
					...
					$st = $this->infobase_->prepare("INSERT INTO products_selection_by_properties_setup_registry (${gf}) VALUES (:${gv})");
		...
		if( config::$analyze_sqlite_tables )
			$this->infobase_->exec('ANALYZE products_selection_by_properties_setup_registry');
		...
		if( config::$log_timing ) {
	    	...
	    	error_log(sprintf('%u', $cnt) . ' products selection by properties setup registry updated, ' . $rps . ' rps, ellapsed: ' . $timer->ellapsed_string($ellapsed));
