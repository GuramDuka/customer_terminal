...
////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------
class customers_loader extends objects_loader {
	...
	public function load_objects() {
		...
		foreach( $this->objects_ as $object ) {
			...
			if( @$erase ) {
				...
				if( $st_erase === null ) {
					...
					$st_erase = $this->infobase_->prepare('DELETE FROM customers WHERE uuid = :uuid');
				...
				if( $st === null ) {
					...
					$st = $this->infobase_->prepare("REPLACE INTO customers (${gf}) VALUES (:${gv})");
		...
		if( config::$analyze_sqlite_tables )
			$this->infobase_->exec('ANALYZE customers');
		...
		if( config::$log_timing ) {
	    	...
	    	error_log(sprintf('%u', $cnt) . ' customers updated, ' . $rps . ' rps, ellapsed: ' . $timer->ellapsed_string($ellapsed));
