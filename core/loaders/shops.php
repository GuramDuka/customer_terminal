...
////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------
class shops_loader extends objects_loader {
	...
	public function load_objects() {
		...
		$all_fields = [ 'uuid', 'marked', 'code', 'name' ];
		$fields = [];
		$fields_uuid = [];
		...
		foreach( $this->objects_ as $object ) {
			...
			if( @$erase ) {
				...
				if( $st_erase === null ) {
					...
					$st_erase = $this->infobase_->prepare('DELETE FROM shops WHERE uuid = :uuid');
				...
				if( $st === null ) {
					...
					$st = $this->infobase_->prepare("REPLACE INTO shops (${gf}) VALUES (:${gv})");
		...
		if( config::$analyze_sqlite_tables )
			$this->infobase_->exec('ANALYZE shops');
		...
		if( config::$log_timing ) {
	    	...
	    	error_log(sprintf('%u', $cnt) . ' shops updated, ' . $rps . ' rps, ellapsed: ' . $timer->ellapsed_string($ellapsed));
