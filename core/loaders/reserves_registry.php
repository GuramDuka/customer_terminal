...
class reserves_registry_loader extends objects_loader {
	...
	public function load_objects() {
		...
		$st_totals_op = <<<'EOT'
			...
			SELECT
				r.product_uuid,
				(COALESCE(t.quantity, 0) * 1000 ${op} r.quantity * 1000) / 1000
			FROM
...
EOT
		...
		if( config::$analyze_sqlite_tables ) {
			...
			$this->infobase_->exec('ANALYZE reserves_records_registry');
			$this->infobase_->exec('ANALYZE reserves_registry');
		...
		$trigger->event([ 'reserves' => array_keys($event) ]);
		$trigger->fire();
